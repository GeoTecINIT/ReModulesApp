...
@Component({
  selector: 'app-home-bp',
  templateUrl: './home-bp.component.html',
  styleUrls: ['./home-bp.component.scss']
})
export class HomeBpComponent implements OnInit {

  // general variables
  history: ceeBuilding[];
  totalHistory: ceeBuilding[];
  building: Building;
  buildingCee: ceeBuilding;
  error: string;
  modalRef: BsModalRef;
...
/*this.building =  new ceeBuilding('', '', '',  null, '', '', '', '',
...
receiveBuildingToCEE($event): void {
  this.active = 1;
  this.showMap = true;
  this.showBuildingInfo = true;
  this.building = $event;
  this.optionSelected = 8;
  this.stepSelected = 'formCEE';
}
...
receiveCeeBuilding($event) {
  this.buildingCee = $event;
  //this.building.refurbishment = new Refurbishment([], [], new SystemType('', '', '', []), new SystemType('', '', '', []));
  this.optionSelected = 2;
  this.active = 1;
  this.stepSelected = 'result';
